...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_turnip_text", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_turnip_text": ["*.pyi", "py.typed"],
        "tree_sitter_turnip_text.queries": ["*.scm"],
    },
    ext_package="tree_sitter_turnip_text",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_turnip_text/binding.c",
                "src/parser.c",
            ],
            define_macros=[
