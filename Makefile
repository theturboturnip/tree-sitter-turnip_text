LANGUAGE_NAME := tree-sitter-turnip-text
HOMEPAGE_URL := https://github.com/theturboturnip/tree-sitter-turnip_text
VERSION := 0.1.0
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/turnip-text '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/turnip-text
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/turnip-text
