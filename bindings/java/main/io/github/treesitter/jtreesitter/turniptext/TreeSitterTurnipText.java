package io.github.treesitter.jtreesitter.turniptext;

import java.lang.foreign.*;

public final class TreeSitterTurnipText {
    ...
    private static final TreeSitterTurnipText INSTANCE = new TreeSitterTurnipText();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterTurnipText() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_turnip_text");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-turnip-text");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-turnip-text");
